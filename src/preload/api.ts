...
export interface ExplorerApi {
  ...
  readonly app: {
    ...
    signalReady(): void
    /** 스플래시 "앞으로 보지 않기"로 showPromoSplash 설정이 바뀌면 푸시(설정 화면 동기화). */
    onPromoSplashChanged(cb: (evt: PromoSplashChangedEvt) => void): Unsubscribe
  }
...
export const api: ExplorerApi = {
  ...
  app: {
    ...
    onPromoSplashChanged: (cb) => subscribe(CHANNELS.APP_PROMO_SPLASH_CHANGED, cb)
  },
