...
| [ADR-013](./ADR-013-shell-context-menu-verbs.md) | Windows 셸 컨텍스트 메뉴 연동(§Y1) | COM `Shell.Application` `FolderItem.Verbs()`/`verb.DoIt()` + **상주 PowerShell 자식 프로세스**(stdin/stdout JSON·명령행 합성 0·경로는 stdin 페이로드)·네이티브 N-API/레지스트리 정적 비채택(신규 네이티브 의존성 0)·verb 식별=index+표시명 결합(실행 시 재열거 교차검증·오실행 방지)·블랙리스트=canonical 부재→표시명 정규화 매칭(영/한·`open/cut/copy/paste/delete/rename/properties/link`)·단일 선택 한정·fire-and-forget·신규 채널 2종(`shell:context-verbs`/`shell:invoke-verb`)·ADR-005 보안 모델 준수 | 제안(🔜 설계 완료·구현 전) |
...
> **ADR-013(2026-06-12 §Y Windows 셸 컨텍스트 메뉴 연동)** 도 ADR-005를 전면 준수한다(sender·zod·guardPath·로컬 한정·**명령행 합성 0**=경로를 stdin JSON으로만 전달·실행 표면 미추가=탐색기와 동일 신뢰 모델). 신규 네이티브/npm 의존성 0(COM·PowerShell 시스템 내장). `showProperties`(COM `Verbs()`/`DoIt()`)·hash/grep 워커(수명·취소)·B6 컨텍스트 메뉴 인프라의 비파괴 확장이다. 상태 🔜 설계 완료·구현 전.
