## 아이디어 백로그 
아래 골든 데이터 셋을 만족할때까지 프롬프트와 툴을 개선하고 추가한다.

### 제약 조건 
- 질의 의도를 분석해 계획을 새우고 계획별 목표 달성을 위해 ReAct 루프를 실행 해서 결과까지 진행할 것 
- 목표를 달성하기위해 후보 디렉토리를 선별하고 파일의 내용을 분석하고 질의와의 연관성을 확인하는 루프(재귀)를 계획할 수 있어야 한다.

### 골든 데이터 셋
질의 : "E: 드라이브에서 브라우저 기반 ERD 설계 도구개발 프로젝트를 찾아 해당 폴더를 열어줘" 
결과 : 새탭이 열리고 "E:\03.프로젝트\SimJaeSugn.github.io" 경로로 이동
