...
> **추가(2026-06-07, L장 그리드 이미지 썸네일 — 구현 완료 ✅):** **신규 1기능(L장: feat-L1 그리드 보기 이미지 썸네일 자체 생성 · 에픽11 US-11.1, PRD §6 Should)이 구현·QA PASS** 됨(아래 "신규 그리드 이미지 썸네일(L장)" 표 참조). **이로써 직전까지 §0.5 "P6 미구현 잔여 🔜"·"P6 잔여 표"에 남아 있던 마지막 항목 "그리드/썸네일 보기 — 이미지 썸네일 자체 생성"이 feat-L1로 구현되어 P6 잔여→완료로 해소**됐다(P6 잔여 표는 이제 비었다). **이로써 개발 잔여(🔜) 0 — 남은 것은 P7 릴리스 실측·실제 코드서명·NSIS 실측·런타임 스모크(🟡)뿐이다(P7 전체는 여전히 🟡, L1로 안 바뀜).** **L1은 신규 채널 `preview:thumbnail`(invoke) 1종을 추가**(P1 동결 이후 Should 신기능 신규 채널 — `preview:read`·`shell:open-terminal`·`analyze:scan:*`·`fs:watch:*`·`trash:*` 선례와 동일 invoke·guard/zod/Result 규약·sender 검증·guardPath·size 버킷 화이트리스트). 신규 파일: `src/main/os/thumbnail.ts`(nativeImage `createFromPath`→**비율 보존 resize**(종횡비로 긴 변=size 단일 축 지정)→`toDataURL`·30MB 상한·LRU 256·실패 비캐싱·세마포어 4·헤드리스 주입)·`src/renderer/infra/icon/thumbnailCache.ts`(iconCache 동형·negCache·in-flight 디듀프·useSyncExternalStore)·`src/renderer/domain/image.ts`(`isThumbnailableExt`·DPR 버킷 `thumbSizeFor`)·`FileListView.tsx ThumbnailIcon`(그리드 icons-* 한정·이미지=`<img objectFit:contain>`·미지원/null/대용량=OSIcon 폴백·details/list는 OS 아이콘 유지). 가드: `guard.ts#zThumbnailReq`·`THUMB_SIZE_BUCKETS [32,48,64,96,128]`. 보안: dataUrl만(CSP `img-src data:`)·size 버킷 화이트리스트(DoS 차단)·guardPath·렌더러 직접 파일 접근 없음(ADR-005·H6 동일 모델). **신규 의존성 0**(Electron 내장 nativeImage). 신규 검증 `verify:thumbnail` 33(비율 보존 단일축 resize·폴백 매트릭스[미지원/빈/대용량]·LRU evict·실패 비캐싱·세마포어). **정직 한계(은폐 금지): nativeImage 실 디코드·GUI 그리드 렌더(`<img>` 교체·가시 셀 지연·1만 항목 비차단)는 런타임 스모크 권장(코드 정합·typecheck/lint 0·`verify:thumbnail` 33 충족).**
