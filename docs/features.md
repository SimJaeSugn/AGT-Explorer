...
> **되돌리기(B7·K1 `Ctrl+Z` 다단계 undo)·휴지통 관리 화면(B5·K2)은 §K로, 그리드 보기 이미지 썸네일 자체 생성(B1·feat-L1 §L)은 §L로 구현 완료 ✅**(아래 §K·§L). **이로써 사용자 기능 Should/C/W 잔여(🔜) 0** — 보기 5종 J3 ✅ + 이미지 내용 썸네일 L1 ✅. 남은 것은 P7 릴리스 실측·코드서명 등 릴리스 안정화 잔여(🟡)뿐이다. 상세 현황은 [roadmap.md §0.5](./roadmap.md).
...
> **이것이 개발 잔여의 사실상 마지막 사용자 기능 항목이었고, 이제 구현 완료되어 사용자 기능 잔여(🔜)가 0이 됐다**(P7 릴리스 실측·코드서명 등 릴리스 안정화 잔여는 별개로 🟡). 단축키는 [PRD.md 8장](./PRD.md#8-단축키-체계-확정--충돌-없음)이 단일 출처다(L1은 보기 동작이라 신규 단축키 불요).
